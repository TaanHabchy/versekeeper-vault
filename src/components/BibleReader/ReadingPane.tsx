...
export const ReadingPane = ({ 
  ...
  if (!book || !chapter || !verses) {
    return (
      <div className="flex-1 flex items-center justify-center bg-background">
        <p className="text-muted-foreground">Select a book and chapter to read</p>
      </div>
    );
  }
  ...
  const handleSaveVerse = () => {
    if (selectedVerse && selectedFolderId && book && chapter) {
      onSaveVerse(book, parseInt(chapter), parseInt(selectedVerse.verse), selectedVerse.text, selectedFolderId);
  ...
  return (
    <div className="flex-1 flex flex-col bg-background">
      <div className="border-b border-border p-4 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">{book} {chapter}</h1>
        </div>
          ...
          {Object.entries(verses).map(([verse, text]) => {
            ...
            return (
              <div key={verse} className="group flex gap-4 items-start">
                <span className="text-sm font-semibold text-primary min-w-[2rem]">{verse}</span>
                <div className="flex-1 flex items-start gap-2">
                  <p className="text-foreground leading-relaxed flex-1">{text}</p>
                  <Dialog open={selectedVerse?.verse === verse} onOpenChange={(open) => !open && setSelectedVerse(null)}>
                    ...
                    <DialogContent>
                      ...
                      <div className="space-y-4">
                        ...
                        <Button onClick={handleSaveVerse} disabled={!selectedFolderId} className="w-full">
                          Save to Collection
                        </Button>
                      </div>
                    </DialogContent>
